...
class LaunchArbiter : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (BuildConfig.DEBUG && forcedNow.isNotBlank()) {
            ...
            debugToast("flux: force → $forcedNow")
        ...
        // NATIVE lock: once native, stay native. In DEBUG this lock is off so
        // developers can retry OneLink / config after a bad run without
        // uninstalling; in RELEASE it is invariant (Play-review requirement).
        ...
        if (store.stage == FluxStore.Stage.NATIVE && BuildConfig.DEBUG) {
            ...
            debugToast("flux: NATIVE lock ignored (debug)")
        ...
        if (pushed != null) {
            Tracer.i(TAG, "cold push URL captured")
    ...
    private suspend fun route() {
        ...
        if (!cold.isNullOrBlank() && HostGate.admits(cold)) {
            Tracer.i(TAG, "cold push URL → STREAM directly")
    ...
    private suspend fun firstLaunch() {
        ...
        when (val outcome = askUplink(attribution)) {
            is StageDecision.Stream -> {
                ...
                debugToast("flux: STREAM ${outcome.url}")
            ...
            is StageDecision.Native -> {
                when {
                    !outcome.answered -> {
                        ...
                        debugToast("flux: uplink unreachable → NATIVE (not persisted)")
                    }
                    attribution.isEmpty() -> {
                        Tracer.i(TAG, "no attribution behind the answer → game, decision open")
                        debugToast("flux: attribution empty → NATIVE (not persisted)")
                    }
                    else -> {
                        store.stage = FluxStore.Stage.NATIVE
                        Tracer.i(TAG, "backend → NATIVE")
                        debugToast("flux: backend said NATIVE (persisted)")
                    }
                }
                handOffToGame()
            }
    ...
    private fun extractPushUrl(source: Intent): String? {
        val own = if (source.getBooleanExtra(EXTRA_FROM_PUSH, false))
            source.getStringExtra(EXTRA_PUSH_URL) else null
        val raw = source.getStringExtra(FCM_KEY_URL) ?: source.getStringExtra(FCM_KEY_LINK)
        return (own ?: raw)?.trim()?.takeIf { it.isNotBlank() && HostGate.admits(it) }
    }
    ...
    override fun onNewIntent(newIntent: Intent) {
        ...
        when (store.stage) {
            FluxStore.Stage.NATIVE -> {
                Tracer.i(TAG, "push tap while NATIVE — game stays open")
                return
            }
